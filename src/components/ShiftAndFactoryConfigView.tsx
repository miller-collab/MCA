...
import { 
  Clock, Save, UserPlus, Users, Check, Trash2, AlertTriangle, Lock
} from 'lucide-react';
...
export const ShiftAndFactoryConfigView: React.FC<ShiftAndFactoryConfigViewProps> = ({
  ...
  // Se bloqueado, mostra tela de PIN
  if (!isUnlocked) {
    return (
  ...
  // Salvar tudo: Turnos + Colaboradores com persistência total
  const handleSaveAll = () => {
    ...
    try {
      ...
      localStorage.setItem('mca_colabs_v3', JSON.stringify(localColabs));
    } catch {}
    ...
    <div className="max-w-[1200px] mx-auto p-3 sm:p-4 space-y-6 animate-in fade-in duration-200">
      ...
      <div className="border-b border-[#333333] pb-3 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-xl font-bold text-white flex items-center gap-2">
            ...
            <span>Configuração de Horários da Fábrica e Turnos</span>
          </h2>
          <p className="text-xs text-[#888888]">
            Defina os horários de início e término dos turnos da fábrica e gerencie os colaboradores cadastrados.
          </p>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={handleSaveAll}
            className="px-5 py-2.5 bg-[#00E676] hover:bg-[#00c853] text-black font-black text-xs sm:text-sm rounded-lg border border-[#00c853] shadow-lg transition flex items-center gap-2 cursor-pointer active:scale-95"
          >
      ...
      {savedSuccess && (
        <div className="p-3.5 bg-[#00E676]/20 border border-[#00E676] rounded-lg text-[#00E676] text-xs sm:text-sm font-bold flex items-center gap-2 animate-in fade-in shadow-lg">
          ...
          <span>Configurações dos Turnos e Colaboradores foram salvas e sincronizadas com sucesso com todos os tablets!</span>
        </div>
      )}
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-1.5">
            ...
            <span>1. Configuração dos Turnos</span>
          </h3>
          <span className="text-[11px] text-[#888888]">
            Horários de entrada, saída e dias de atuação por turno
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {localShifts.map((shift, idx) => {
            ...
            // Carga horária líquida direta: Entrada até Saída
            const totalLiquido = calcularDiferencaMinutos(shift.entrada, shift.saida);
              ...
              >
                ...
                <div className="card-body p-4 text-left space-y-3.5">
                  ...
                  <div className="bg-[#1A1A1A] p-2.5 rounded border border-[#333333] text-center">
                    <div className="text-[10px] text-[#888888] font-bold">Carga Horária Útil</div>
      ...
      {/* Botão de Salvar em Destaque no Meio */}
      <div className="max-w-md mx-auto pt-2">
        <button
          onClick={handleSaveAll}
          className="btn w-full py-3.5 bg-[#00E676] hover:bg-[#00c853] text-black font-black text-base rounded-lg border border-[#00c853] shadow-xl transition flex items-center justify-center gap-2 cursor-pointer active:scale-[0.99]"
        >
      ...
      <div className="pt-6 border-t border-[#333333] space-y-4">
        ...
        {colabSuccess && (
          <div className="p-3 bg-[#00E676]/20 border border-[#00E676] rounded-lg text-[#00E676] text-xs font-bold flex items-center gap-2 animate-in fade-in">
          ...
          >
            <div>
              ...
              <input
                ...
                value={newColabName}
                onChange={(e) => setNewColabName(e.target.value.toUpperCase())}
                className="w-full p-2 bg-[#222222] text-white border border-[#555555] rounded text-xs focus:outline-none focus:border-[#007BFF] uppercase"
                required
              />
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2.5">
          {localColabs.map((c) => {
            ...
            return (
              <div
                key={c.id}
                className="p-3 bg-[#111111] border border-[#333333] rounded-lg flex items-center justify-between gap-2 shadow-xs"
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                  {onToggleCollaboratorActive && (
                    <button
                      ...
                      className={`px-2.5 py-1 rounded text-[10px] font-bold cursor-pointer transition ${
                        c.active
                          ? 'bg-[#00E676]/20 text-[#00E676] border border-[#00E676]/40 hover:bg-[#00E676]/30'
                          : 'bg-[#555555]/20 text-[#888888] border border-[#555555] hover:bg-[#555555]/30'
                      }`}
                    >
                  ...
                  {onDeleteCollaborator && (
                    <button
                      ...
                      className="p-1.5 hover:bg-[#333333] text-[#888888] hover:text-[#FF3D00] rounded transition cursor-pointer"
